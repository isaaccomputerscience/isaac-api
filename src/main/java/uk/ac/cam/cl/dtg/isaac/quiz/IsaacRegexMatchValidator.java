...
public class IsaacRegexMatchValidator implements IValidator {
  ...
  public final QuestionValidationResponse validateQuestionResponse(final Question question, final Choice answer) {
    ...
    if (!(answer instanceof StringChoice)) {
      throw new IllegalArgumentException(
        String.format(
          "Expected StringChoice for IsaacRegexMatchQuestion: %s. Received (%s) ",
          question.getId(),
          answer.getClass()
        )
      );
    ...
    if (null == feedback) {
      ...
      for (Choice c : orderedChoices) {
        ...
        if (!(c instanceof RegexPattern)) {
          log.error(
            "Isaac RegexMatch Validator for questionId: " +
        ...
        if (null == regexPattern.getValue() || regexPattern.getValue().isEmpty()) {
          log.error(
            "Expected a regex pattern to match on, but none found in choice for question id: " +
            regexMatchQuestion.getId()
          );
          continue;
        }
        ...
        if (
          matchesPattern(
            regexPattern.getValue(),
            userAnswer.getValue(),
            regexPattern.isCaseInsensitive(),
            regexPattern.isMultiLineRegex(),
            regexPattern.isMatchWholeString()
          )
        ) {
  ...
  private boolean matchesPattern(
    final String trustedRegexPattern,
    final String userValue,
    final Boolean caseInsensitive,
    final Boolean multiLineRegex,
    final Boolean matchWholeString
  ) {
    ...
    Pattern answerPattern = Pattern.compile(
      Pattern.quote(trustedRegexPattern),
