...
public class GameboardsFacade extends AbstractIsaacFacade {
   ...
   * @param title              - the title of the generated board
   ...
   * @param concepts           - a comma separated list of conceptIds
   * @param questionCategories - a comma separated list of question categories
  ...
  public final Response generateTemporaryGameboard(
    ...
    List<String> stagesList = QueryUtils.splitCsvStringQueryParam(stages);
    List<String> difficultiesList = QueryUtils.splitCsvStringQueryParam(difficulties);
    List<String> examBoardsList = QueryUtils.splitCsvStringQueryParam(examBoards);
    List<String> conceptsList = QueryUtils.splitCsvStringQueryParam(concepts);
    List<String> questionCategoriesList = QueryUtils.splitCsvStringQueryParam(questionCategories);
   ...
   *
   * @param request            - so that we can deal with caching and etags.
   * @param httpServletRequest - so that we can extract the users session information if available.
   * @param gameboardId        - the unique id of the gameboard to be requested
   * @return a Response containing a gameboard object or containing a SegueErrorResponse.
   */
  @GET
   ...
   * @param newGameboardObject - the new gameboard to save in the database
   ...
   * @param newGameboardTitle - a string containing the new title to save the gameboard with
   ...
   *
   * @param request          - so that we can find out the currently logged in user
   * @param startIndex       - the first board index to return.
   * @param limit            - the number of gameboards to return.
   * @param sortInstructions - the criteria to use for sorting. Default is reverse chronological by created date.
   * @param showCriteria     - e.g. completed,incompleted
   ...
   * @param gameboardIds - comma separated list of gameboard ids to unlink
