...
public class ExceptionSanitiser implements ContainerResponseFilter {
  ...
  public void filter(
    ...
    throws IOException {
    if (
      containerResponseContext.getEntityType() == SegueErrorResponse.class &&
      ...
      containerResponseContext.setEntity(
        new SegueErrorResponse(error.getResponseCode(), error.getResponseCodeType(), responseMessage, null)
      );
    }
  }
}
