...
public class UsersFacade extends AbstractSegueFacade {
   ...
   * @param recaptchaManager       - so we can check recaptcha token is valid
   * @param logManager             - so we can log interesting events.
   * @param userAssociationManager - so we can check permissions..
   ...
   * @param httpServletRequest - the request which may contain session information.
   * @param response           - the response to set session expiry information headers on.
   ...
   * @param userObjectString - object containing all user account information including passwords.
   ...
   *
   * @param token          - A password reset token
   * @param clientResponse - A user object containing password information.
   * @param request        - For logging purposes.
   ...
   *
   * @param httpServletRequest Authentication and authorisation
   * @param userIdsQueryParam  The comma seperated list of user ids to try and find schools for.
