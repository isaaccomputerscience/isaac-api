...
public class QuizFacadeTest extends AbstractFacadeTest {
  ...
  public void setUp() throws ContentManagerException {
    ...
    expect(contentSummarizerService.extractContentSummary(studentQuiz, QuizSummaryDTO.class)).andStubReturn(
        studentQuizSummary);
    expect(contentSummarizerService.extractContentSummary(teacherQuiz, QuizSummaryDTO.class)).andStubReturn(
        teacherQuizSummary);
    ...
    replay(requestForCaching, properties, logManager, contentManager, contentSummarizerService, quizManager,
        groupManager, quizAssignmentManager,
...
//  }
  ...
  private QuizFeedbackDTO getFeedbackFor(RegisteredUserDTO student) {
    return studentAssignment.getUserFeedback().stream().filter(f -> f.getUser().getId().equals(student.getId()))
  ...
  public void updateQuizAssignment() {
    ...
    forEndpoint(
        (updates) -> () -> quizFacade.updateQuizAssignment(httpServletRequest, studentAssignment.getId(), updates),
