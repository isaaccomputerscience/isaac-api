...
// Note: Parameterised testing does not appear to be compatible with the older JUnit4 annotations
// For the moment, this has been defined seperately but existing test files should probably be transitioned as appropriate
/**
 ...
 */
public abstract class IsaacIntegrationJupiterTest {
    ...
    public static void setUpClass() {
        postgres = new PostgreSQLContainer<>("postgres:12")
                ...
                .withFileSystemBind(IsaacIntegrationJupiterTest.class.getClassLoader().getResource("db_scripts/postgres-rutherford-create-script.sql").getPath(), "/docker-entrypoint-initdb.d/00-isaac-create.sql")
                .withFileSystemBind(IsaacIntegrationJupiterTest.class.getClassLoader().getResource("db_scripts/postgres-rutherford-functions.sql").getPath(), "/docker-entrypoint-initdb.d/01-isaac-functions.sql")
                .withFileSystemBind(IsaacIntegrationJupiterTest.class.getClassLoader().getResource("db_scripts/quartz_scheduler_create_script.sql").getPath(), "/docker-entrypoint-initdb.d/02-isaac-quartz.sql")
                .withFileSystemBind(IsaacIntegrationJupiterTest.class.getClassLoader().getResource("test-postgres-rutherford-data-dump.sql").getPath(), "/docker-entrypoint-initdb.d/03-data-dump.sql")
        ;
